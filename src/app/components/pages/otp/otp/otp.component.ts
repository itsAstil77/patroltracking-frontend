...
export class OtpComponent {
  ...
  onSubmit() {
    ...
    this.http.post("http://172.19.9.152.31:5000/login/verify-otp", this.otpObj)
      .subscribe({
        next: (res: any) => {
          if (res.success) {
  ...
  onResendOTP() {
    ...
    this.http.post("http://172.19.9.152.31:5000/api/auth/resend-otp", { email: this.otpObj.email })
      .subscribe({
        next: (res: any) => {
